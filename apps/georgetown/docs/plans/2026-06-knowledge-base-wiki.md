...
**App**: Georgetown
**Owner**: CTO
**Appetite**: Small batch (~1 day, 2–3 sessions)
...
- **No starting the build before the editor decision (R4 blocking gate).** The first code step (`pnpm add @blocknote/*`) is downstream of an unanswered question: BlockNote, or reuse Georgetown's existing TipTap? Do not `pnpm add` anything until the CEO has answered the R3/R5 test. This is the one hard precondition — everything else in the pitch assumes it's resolved.
- **No three-state visibility without the CEO visibility decision (R4).** Default to a single `is_public` boolean unless the CEO explicitly wants per-page member-vs-public as a routine author choice. Building the enum + its RLS branch speculatively is the R3 over-engineering trap.
