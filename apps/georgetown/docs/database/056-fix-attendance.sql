-- Migration 056: Attendance Records and Statistics (FIXED FOR ACTUAL SCHEMA)
-- Purpose: Track actual meeting attendance + calculate stats
-- Date: 2025-12-02
-- Fixes: events.type (not event_type), members.active (not status)
...
CREATE INDEX idx_attendance_event_member ON attendance_records(event_id, member_id);
...
COMMENT ON TABLE member_attendance_stats IS 'Pre-calculated attendance statistics per member';

-- ============================================================================
-- 3. ATTENDANCE SUMMARY VIEWS (FIXED)
-- ============================================================================
...
SELECT
  ...
  e.location_id AS event_location_id,  -- Fixed: location_id not location
  ...
  (SELECT COUNT(*) FROM members WHERE active = true) AS total_active_members,  -- Fixed: active boolean

  -- Attendance percentage (members only)
  ROUND(
    (COUNT(*) FILTER (WHERE a.attendee_type = 'member')::DECIMAL /
    NULLIF((SELECT COUNT(*) FROM members WHERE active = true), 0) * 100),  -- Fixed: active boolean
    1
...
COMMENT ON VIEW meeting_attendance_summary IS 'Per-meeting attendance summary';

-- View: Member attendance detail
...
WHERE e.type = 'club_meeting'  -- Fixed: type not event_type
  AND m.active = true  -- Fixed: active boolean
ORDER BY e.date DESC, m.name;

COMMENT ON VIEW member_attendance_detail IS 'Detailed member attendance with RSVP correlation';
...
WHERE m.active = true  -- Fixed: active boolean
...
COMMENT ON VIEW at_risk_members IS 'Members below 60% attendance or 4+ consecutive absences';
...
-- Function: Refresh member attendance stats (FIXED)
...
BEGIN
  ...
  SELECT
    ...
    (
      SELECT COUNT(*)
      FROM events e2
      WHERE e2.type = 'club_meeting'  -- Fixed: type not event_type
  ...
  FROM members m
  LEFT JOIN events e ON e.type = 'club_meeting' AND e.date <= CURRENT_DATE  -- Fixed: type not event_type
