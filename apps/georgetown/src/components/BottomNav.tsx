...
export default function BottomNav() {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 z-50 h-16 safe-area-inset-bottom">
            ...
            <Link
              ...
              className={`flex flex-col items-center justify-center gap-1 px-2 sm:px-3 py-2 rounded-lg transition-all duration-200 min-w-[60px] relative ${
                isActive
                  ...
                  : 'text-gray-600 hover:text-[#0067c8] hover:bg-gray-50'
              }`}
              aria-label={item.label}
              aria-current={isActive ? 'page' : undefined}
            >
              {isActive && (
                <div className="absolute top-0 left-1/2 -translate-x-1/2 w-8 h-1 bg-[#0067c8] rounded-b-full" />
              )}
              <Icon
                size={20}
                strokeWidth={2}
